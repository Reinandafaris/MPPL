...
<form action="{{ route('kaskeluar.update', $kaskeluar->id) }}" method="POST" enctype="multipart/form-data">
  ...
  <fieldset>
    <div class="form-group row">
      <div class="col-md-5">
        Nomor Nota<input id="notran" type="text" name="notrans" class="form-control" value="{{ $kaskeluar->nokk }}" required>
      </div>
      <div class="col-md-5">
        Tanggal Transaksi<input id="tgltr" type="date" name="tgltr" value="{{ $kaskeluar->tglkk }}" class="form-control"
          required>
      </div>
    </div>
    <div class="form-group row">
      <div class="col-md-10">
        Memo
        <textarea id="memo" type="text" name="memo" class="form-control" required>{{ $kaskeluar->memokk }}</textarea>
      </div>
    </div>
    <div class="form-group row">
      <div class="col-md-10">
        Tempat Beli
        <input id="tmptbeli" type="text" name="tmptbeli" class="form-control" value="{{ $kaskeluar->tmptbeli }}" required>
      </div>
    </div>
    <hr>
    ...
    <div class="form-group row">
      <div class="col-md-6">
        ...
        @for ($i = 1; $i <= 3; $i++)
          ...
          <option value="{{ $akn->id }}"
            @if(isset($kaskeluardet[$i-1]) && $kaskeluardet[$i-1]->idakun == $akn->id) selected @endif>
      ...
      <div class="col-md-4">
        ...
        @for ($i = 1; $i <= 3; $i++)
          ...
          value="{{ isset($kaskeluardet[$i-1]) ? $kaskeluardet[$i-1]->nilcr : 0 }}" onkeyup="sum();">
          @endfor
          <input id="idkk" type="hidden" name="idkk" class="form-control" value="{{ $kaskeluar->id }}" required>
      </div>

    </div>
    <div class="form-group row">
      <div class="col-md-10">Total Penerimaan
        <input id="total" type="text" name="total" class="form-control" value="{{ $kaskeluar->jmkk }}" required>
      </div>
    </div>
